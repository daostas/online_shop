...
func main() {
	...
	logf, err := os.OpenFile("./prod-svc/log/qs.log", os.O_CREATE|os.O_RDWR|os.O_APPEND, 0666)
	if err != nil {
	...
	logger := log.New(logf, "Prod Service: ", log.Flags())
	...
	cfg, err := config.LoadConfig("./prod-svc/config")
	if err != nil {
		logger.Fatalf("Error loading loadConfig: %v", err)
	}

	s := grpc.NewServer()
	srv := prod_service.NewProdServiceServer(Db, &cfg)
	...
	go func() {
		logger.Println("Prod Svc on", cfg.Port)
		fmt.Println("Prod Svc on", cfg.Port)
		err := s.Serve(lis)
		if err != nil {
			log.Fatal(err)
		}
